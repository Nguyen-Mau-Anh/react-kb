...
- [x] `13.service-stack/02-graphql.md` — GraphQL with React: `@apollo/client@3.11` vs `urql@4` vs `relay@18`, codegen, subscriptions
- [x] `13.service-stack/03-background-jobs.md` — `inngest@3` vs `@trigger.dev/sdk@3` vs `bullmq@5` vs `@upstash/qstash@2` for async work
- [x] `13.service-stack/04-search.md` — Algolia vs Typesense vs MeiliSearch vs Postgres full-text + `pg_trgm`
- [x] `13.service-stack/05-headless-cms.md` — `sanity@3` vs Contentful vs `payload@3` vs `strapi@5` for content-driven React/Next.js

---

## Curriculum Closed (55 topics across 13 groups)

The full top-to-bottom React + Next.js + ecosystem path. To extend later: append a new
`## Group 14 — <theme>` with five `[ ]` items and a matching folder. Same conventions apply.
